    ...
    namespace BulletHeck
    {
        namespace Patterns
        {
            namespace Shape
            {
                ...
                public class ComplexNSidedImplementation : IShapeImplementation
                {
                    public void Build(Vector3 worldPosition, ShapePattern shapePattern, ref ShapePoint[] shapePoints)
                    {
                        ...
                        MathHelper.GetComplexPointsOnMultipleRadius(ref shapePoints, worldPosition, shapePattern, 0.0f, false, true);
                    }
                    ...
                    public void OnSelectedGizmoDraw(Vector3 origin, Vector3 forward, ShapePattern shapePattern, ref ShapePoint[] shapePoints)
                    {
                        for(int i = 0; i < shapePoints.Length; ++i)
                        {
                            ...
                            {
                                Gizmos.DrawLine(shapePoints[i].world, shapePoints[i + 1].world);
                            }
                            else
                            {
